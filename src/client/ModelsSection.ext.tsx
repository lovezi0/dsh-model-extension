...
import { ADAPTER_VERSION } from './extension-meta.ts'
 ...
 * renderer erases the share boundary at the render call). EXT: no child-slot
 * dispatch seat — see the header note.
 */
  ...
  return (
    <div className={styles['section']}>
      <h2 className={styles['title']}>
        {t('title')}
        {/* EXT-only: adapter anchor badge (upstream has no counterpart). */}
        <span className={styles['intro']}>{` — ${t('adapterVersion').replace('{version}', ADAPTER_VERSION)}`}</span>
      </h2>
