...
import { ModelExtensionFields } from './ModelExtensionFields.tsx'
      ...
      {models.map((model, index) => (
        <div key={index} className={styles['modelEntry']}>
          ...
          {expanded.has(index)
            ? (
              <>
                {/* Host `.modelAdvanced` is `display:grid` with
                    `repeat(auto-fit, minmax(160px, 1fr))`; any child placed
                    INSIDE it is squeezed into one narrow grid column. The
                    extension fields need the full entry width, so they render
                    as a SIBLING of `.modelAdvanced` — a direct child of the
                    block-level `.modelEntry` — rather than inside its grid. */}
                ...
                {/* EXT-only: extension fields edit the same draft object in
                    place; the card's own save path persists them untouched.
                    patch is (index, next) — bind this row's index here. */}
                <div className={styles['modelExtensionFields']}>
                  <ModelExtensionFields
                    model={model}
